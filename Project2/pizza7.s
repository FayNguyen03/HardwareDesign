...
pizza_size_prompt_string:
    ...
    .align 2
@ order_more_string: prompt to ask whether the client wants to order one more pizza
order_more_string:
    .ascii "Would you like to order another pizza? (y/n)\000"
    .align 2
...
thank_you_string:
    ...
    .align 2
@ input: input string to get the value whether the client wants to order more
input:
    .ascii "%c\000"
    .align 2
...
@ [fp, #-28] is parameter input - this will be used for both two questions to save up space
...
@ loop body
loop_body:
@ print out the question for client to ask which size of pizza they want to order more pizza
@ load order_more_string_ptr into the register r0
    ldr r0, order_more_string_ptr
    bl printf

@ load input_ptr into the register r0
    ldr r0, input_ptr
@ store the address input into the register r1
    sub r1, fp, #28
@ scan the value
    bl scanf
@ discard the new line char
...
@ loop condition
loop_condition:
@ check if the client wants to order more
@ load value of input into r0
	sub r0, fp, #28
    ldr r0, [r0]
    cmp r0, #'n'
    bne asksize
...
@ order_more_string_ptr: pointer for order_more_string
order_more_string_ptr:
    .word order_more_string
